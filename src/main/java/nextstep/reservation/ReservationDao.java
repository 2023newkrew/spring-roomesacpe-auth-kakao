...
public class ReservationDao {
    ...
    public Reservation findById(Long id, Long memberId) {
        String sql = "SELECT reservation.id, reservation.schedule_id, reservation.member_id, schedule.id, schedule.theme_id, schedule.date, schedule.time, theme.id, theme.name, theme.desc, theme.price " +
                "from reservation " +
                "inner join schedule on reservation.schedule_id = schedule.id " +
                "inner join theme on schedule.theme_id = theme.id " +
                "where reservation.id = ? and reservation.member_id = ?;";
        try {
            return jdbcTemplate.queryForObject(sql, rowMapper, id, memberId);
        } catch (Exception e) {
            return null;
        }
    }
